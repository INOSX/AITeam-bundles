...
- `.memory/kenji.md` — meu próprio histórico (se existir): bundles que já forjei, decisões de naming já tomadas, padrões de glyph que ficaram bons.
...
- **"Cria agente X"** → batch reads em paralelo: `index.json`, schema `agent-bundle.v1.json`, um bundle de referência (linh). Depois forjo.
