...
Echo confirms the user's input verbatim, proposes a next step, and refuses anything destructive. It uses only `read_file` and `list_directory` from the harness tool registry.

## Install on AITEAM-X

From the dashboard: **Bundles → Catalog → Echo → Install**.

Or via API: `POST /api/bundles/install { "id": "echo", "version": "1.0.0" }`.
