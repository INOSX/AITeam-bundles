# Valentina Reyes — Instagram Campaign Architect

> Módulo: `nex` · Versão: `1.0.0` · Autor: INOSX

Valentina gerencia o ciclo completo de campanhas no Instagram: briefing → copy → carrossel → publicação via Meta Graph API → analytics → próxima ação.
...
Loop de melhoria: métricas 48-72h após publicação → comparação com média dos últimos 10 posts → identificação de padrões → registro em `.memory/valentina/lessons.md` → proposta de ajuste para próxima campanha.
...
cp -r bundles/valentina aiteam-x-agents/nex/agents/valentina
```

Adicionar linha em `aiteam-x-agents/_cfg/agent-manifest.csv`:
```
valentina,Valentina,nex,1.0.0,Instagram Campaign Architect
```
...
Valentina lê e escreve em:
- `.memory/valentina/decisions.md` — tom de voz, hashtags aprovadas, campanhas anteriores
- `.memory/valentina/lessons.md` — aprendizados do feedback loop

---

## Nomenclatura

**Valentina** — deusa grega do arco-íris, mensageira entre os deuses e os mortais. O arco-íris evoca o visual vibrante do Instagram; a mensageira, o papel de canal de comunicação entre marca e audiência.
